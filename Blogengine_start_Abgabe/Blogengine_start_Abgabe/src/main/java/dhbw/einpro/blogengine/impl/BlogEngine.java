...
public class BlogEngine implements IBlogEngine {
    ...
    @Override
    @Override
    public boolean addUser(IUser p_user)
        ...
        // 1) DuplicateUser prüfen (gleiche Instanz / equals)
        if (users.contains(p_user)) {
            throw new DuplicateUserException(
                    "Der gleiche User wurde erneut hinzugefügt."
            );
        }

        // 2) DuplicateEmail prüfen
        String emailKey = normalizeEmail(p_user.getEmail());
        if (emailKey != null && usersByEmail.containsKey(emailKey)) {
            throw new DuplicateEmailException(
                    "Ein User mit dieser Email existiert bereits: " + p_user.getEmail()
            );
        }
